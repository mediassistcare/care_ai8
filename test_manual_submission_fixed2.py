...
def test_manual_submission_fixed2():
    """Test manual submission with properly fixed case number generation"""
    
    print("🔄 Testing manual case submission (FIXED v2)...")
    ...
    try:
        ...
        # FIXED: Generate case number correctly (simple approach)
        def generate_case_number_fixed():
            """Generate unique case number (SIMPLE VERSION)"""
            ...
            print(f"📊 Found {len(results)} existing cases for {current_year}:")
            ...
            for result in results:
                ...
                # Try to extract number from case_number
                ...
                if len(parts) >= 3:
                    try:
                        ...
                        print(f"      ➜ could not parse number from: {case_number}")
            
            next_number = max_number + 1
            new_case_number = f"CASE-{current_year}-{next_number:04d}"
            print(f"🎯 Next case number will be: {new_case_number}")
        ...
        print(f"💾 Inserting case into database...")
        ...
        if result:
            ...
            print(f"🎊 SUCCESS! The submission logic works when case number generation is fixed!")
        else:
